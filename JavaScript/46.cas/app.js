...
const niz = [1, 3, 4, 5, 6, 7, 9];
const niz2 = [2, 4, 5, 6, 8, 10];

function proveravanje(arr, arr1) {
  const newarr1 = arr.filter(function (element) {
    return !arr1.includes(element);
  });
  const newarr2 = arr1.filter(function (element) {
    return !arr.includes(element);
  });
  const newarr = newarr1.concat(newarr2);
  return newarr.sort(function (a, b) {
    return a - b; //callback za redosled
  });
}
console.log(proveravanje(niz, niz2));
...
function cene(niz) {
  const niz2 = niz.map(function (element) {
    if (element.quantity > 40) {
      element.price * 0.6;
      return element;
    } else if (element.quantity > 20 && element.quantity < 40) {
      element.price * 0.8;
      return element;
    } else {
      return element;
