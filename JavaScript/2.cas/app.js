// Javascript moze prikazati podatke na nekoliko nacina:

// 1.innerHTML

document.getElementById("p").innerHTML = "Paragraf"

// 2.write()
document.write()//najcesci nacin za testiranje nekog koda

// 3.alert()
alert("Zdravo")

// 4.console.log()-ispisivanje u konzoli 

console.log("Poruka za ispis u konzoli")

// Pravila nazivanja promenljivih:

// Promenljiva u JavaScriptu mora da pocne sa nekom od sledecih znakova.

// 1.Slovo(veliko ili malo ) A-Z ili a-z
// 2.$ znak moze biti pocetni karakter neke promenljive
// 3. _

// Broj moze biti sadrzan u nekoj promenljivoj,ali ne sme promenljiva poceti brojem.

// prom1 = "neki string" jeste korektno 
// 2prom = "neki string" nije korektno

// prosek ocena = 4.14 nije korektno
prosekOcena = 4.14. // preporucljiv nacin zapisivanja promenljivih 
prosek_ocena = 4.14 // takodje je ovo ispravno
ProsekOcena = 4.14 // i ovo je ispravno


// Postoje 4 nacina za deklarisanje neke promenljive:
// 1. prom = 12 - dodeljivanje vrednsti bez da smo prethodno izvrsili deklaraciju 

// 2. koriscenjem var keyword(rezervisane reci) - dodatak
// var je koriscena na starijim browserima  (sve se manje koristi)

// var a = 5

// 3. Koriscenjem let keyword(rezervisane reci)
// let se najcesce koristi kada postoji sansa da se vrednost date promenljive menja.

// 4.Koriscenjem const keyword (rezervisane reci )- 
// const se najcesce koristi definisanje konstanti koje nece menjati vrednost.

// Promenljive odnosno varijable predstavljaju kontejnere za skladistenje vrednost